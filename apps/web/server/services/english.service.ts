...
export const saveEnglishSession = async (
  ...
  const existing = await englishRepo.getEnglishBestScore(studentId, input.minigame, input.level)
  ...
  if (isNewBest) {
    await englishRepo.upsertEnglishBestScore(studentId, input.minigame, input.level, score, stars)
  }

  await englishRepo.addUserPoints(studentId, pointsEarned)
...
export const getTodayEnglishHomework = async (
  ...
  const item = await db.dailyHomework.findFirst({
    where: { studentId, date, subjectId: 'english', isDone: false },
  })
  if (!item) return null
