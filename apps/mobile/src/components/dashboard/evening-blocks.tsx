...
export function WeekEveningSection({
  ...
  return (
    <View className="rounded-[22px] bg-white p-4" style={shadow('sm')}>
      <Text className="mb-3 font-display-bold text-[11px] uppercase text-text-muted">
        🌙 Học thêm buổi tối
      </Text>
        ...
        {activeDays.map((day) => (
          <View
            ...
            <View className="flex-row items-baseline gap-1.5">
              <Text className="font-display-bold text-[11px] uppercase text-text-secondary">
                {DAY_LABELS[day]}
              </Text>
              {dateByDay?.[day] ? (
                <Text className="font-display-semibold text-[10px] text-text-muted">
                  {dateByDay[day]}
                </Text>
              ) : null}
            </View>
