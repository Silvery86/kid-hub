# Responsive Design System — Kid Hub
...
| `landscape:` | orientation | Explicit landscape re-assertion (rare) |
...
- **Icons only** in kid routes — no text labels.
...
- Landscape sidebar must be hidden via `portrait:hidden`; tab bar must be hidden via `landscape:hidden`.

### 4.3 P3 — Desktop / Parental (Tertiary)

- **Pattern:** Fixed sidebar with **icon + label**.
- **Width:** `lg:w-56` (224px).
- **Parent routes only.** Kid route navigation does not change at `lg:`.

---

## 5. Safe Area Rules

### 5.1 Utility Classes in `globals.css`
...
### 5.2 Mandatory Application Rules
...
- **Game canvas:** Must fill 100% of the usable viewport minus the sidebar width and all safe areas. Use `h-dvh` not `h-screen`.
- **No scroll:** Game containers must not overflow. Scrolling inside a game is a design failure.
- **Touch events:** Every interactive game element must have `touch-action: manipulation`.

### 6.2 `(dashboard)` — Kid Home View

- **Cards:** Must be tappable, minimum `min-h-tap-lg` (64px).
- **Scroll allowed** in portrait, but discouraged in landscape.
...
- **PIN entry screen** is the only P1-required screen in the parent route group.
...
- Animations must respect `prefers-reduced-motion`. Wrap all `animate-*` classes with a `motion-safe:` prefix.
- Game animations that are essential to gameplay are exempt but must be kept under 300ms.
