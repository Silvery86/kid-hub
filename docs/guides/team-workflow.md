...
2. **Review the Stability Plan** (`docs/architecture/stability-plan.md`) — confirm the feature does not touch any P0/P1 risk areas without a paired fix.
...
| Feedback flash (game) | `bg-emerald-900/40` (correct) / `bg-red-900/40` (wrong) |
...
- Call `requireParentSession()` at the top of every parent-only action (import from `server/lib/auth-guard.ts`).
...
- [ ] No new P0/P1 stability risks introduced (see `docs/architecture/stability-plan.md`)
...
3. **Regression check** — run the auth middleware tests (`e2e/auth/middleware.spec.ts`) on every merge, regardless of what changed.
...
4. Check `docs/architecture/stability-plan.md` for any P0 items that apply to this deploy.

---

## Security Shield — Pre-push Hook

A pre-push Git hook blocks commits containing hardcoded secrets.
...
- E2E fixture JWTs must be generated at test runtime (not hardcoded).
...
| Put business logic in an action | Actions can't be unit-tested in isolation | `auth.actions.ts:82` |
...
| Concurrent/race | Two simultaneous saves do not corrupt state |
