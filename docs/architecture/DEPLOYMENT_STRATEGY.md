...
| Dimension | Detail |
|---|---|
| Cost | $0 (Workers Free: 100 k req/day) |
...
| When to choose | If Vercel free function timeout causes persistent UX problems; or if cost-per-feature on paid Vercel exceeds €5/month |
...
> **PM ACTION REQUIRED:** Approve code change to `next.config.ts` and `prisma/schema.prisma`
> before the Lead Dev makes the edit.
...
### 2.3 Serverless Driver (optional upgrade path)
If the 10 s timeout becomes a bottleneck and we migrate to Edge runtime in the future,
swap in `@neondatabase/serverless` + `@prisma/adapter-neon`. This is a 2-file change
(`lib/db.ts` + prisma config). No schema migration needed.
...
| Latency | ~10–50 ms from Cloudflare PoPs globally |
...
|---|---|
| Cost | $0 |
| Verdict | ❌ Bloats build artifact; Vercel has a 100 MB compressed build limit; audio files would hit this quickly |
...
[git push to main] ──► GitHub Actions: ci.yml
                    ...
                    │    - Spin up Neon branch (staging branch)         │
                    ...
                    │    - Runs prisma migrate deploy against prod DB   │
                    └───────────────────────────────────────────────────┘
```
...
| Playwright report | Uploaded as Actions artifact (90-day retention) | Free, no external service needed |
...
graph TD
    ...
    VERCEL["Vercel Hobby<br/>Next.js App<br/>(Node.js Serverless)"]
    NEON_PROD["Neon Postgres<br/>Production DB<br/>(pooler endpoint)"]
    NEON_STG["Neon Postgres<br/>Staging Branch<br/>(for Playwright)"]
    R2["Cloudflare R2<br/>Media Storage<br/>(audio + images)"]
    USER["Browser<br/>(Parent + Kid)"]

    DEV -- "git push" --> GH
    GH -- "trigger on push/main" --> GHA
    GHA -- "Job 1: lint + tsc" --> GHA
    GHA -- "Job 2: Playwright tests" --> NEON_STG
    GHA -- "Job 3: vercel --prod" --> VERCEL
    GHA -- "prisma migrate deploy" --> NEON_PROD

    USER -- "HTTPS request" --> VERCEL
    VERCEL -- "Server Actions / Route Handlers" --> NEON_PROD
    USER -- "Static assets (audio, images)" --> R2
```
...
|---|---|---|---|---|
| Vercel | Hobby (Free) | 100 GB bandwidth, 10 s functions | < 1 GB, < 100 req/day | **$0** |
...
**Plan B (Coolify/Hetzner) cost if needed:**
| Service | Plan | Monthly Cost |
|---|---|---|
| Hetzner CX11 VPS | 1 vCPU, 2 GB RAM | €3.29 |
| Neon Postgres | Free tier (same) | $0 |
| Cloudflare R2 | Free tier (same) | $0 |
| **Total** | | **~€3.29 / month** |

---

## 7. Deployment Steps (High-Level — Not Yet Executed)
...
3. **`SESSION_SECRET` rotation plan:** The CLAUDE.md P0 blocker notes the secret is not set in
   the real docker-compose. Before prod deploy, we need a rotation plan for the JWT secret.

4. **Staging environment:** Is a separate Vercel preview deployment (per branch) desired, or
   is a single `main → production` pipeline sufficient?
