...
export const MathGame = ({ initialLevel = 1 }: MathGameProps) => {
    ...
    if (state.status === 'result') {
      ...
      setBestScores((prev) => {
        const filtered = prev.filter(
          (b) => !(b.gameType === 'math' && b.level === state.level),
        );
  ...
  const bestScore =
    bestScores.find((b) => b.gameType === 'math' && b.level === state.level) ?? null;
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      >
        {/* Question card */}
        <div className="rounded-3xl bg-slate-700 px-16 py-10 shadow-2xl text-center animate-in fade-in anim-duration-200">
          <p className="text-8xl font-extrabold text-white tracking-tight select-none">
          ...
          {currentQuestion.options.map((option) => {
            ...
            return (
              <KidButton
                ...
                className={cn(
                  'text-6xl font-extrabold min-w-48 min-h-32 transition-colors duration-200',
